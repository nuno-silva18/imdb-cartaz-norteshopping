...
movies_ratings_list = []
stop_words_movies = [
    '2D', '3D', '4DX', 'ATMOS', 'IMAX', 'VP', 'VO', 'VIP', 'XVision'
]
...
for movie in movies_names_list:
  for stop_word in stop_words_movies:
    if stop_word in movie:
      ...
      movie = re.sub(
          '[($-)]+', '', movie
      )    # Remove parentheses with no chars between them from the titles of the movie
  ...
  if movie_search_results:
    ...
    if movie_obj_id not in movies_id_list:
      ...
      if 'rating' in movie_obj:
        movies_ratings_list.append([
            movie, movie_obj, movie_obj['rating']
        ])    # Movies that have not yet been released will not have a rating
      else:
        movies_ratings_list.append(
            [movie, movie_obj, "This movie has not been released yet!"])
    else:
      movies_ratings_list.append(
          [movie, '', "No information has been found for this movie :("])

msg_text = "Here are this week's movies airing at NorteShopping with an IMDB rating equal to or over 7.5:\n\n"

movies_high_rated_flag = 0
for movie, movie_en, rating in movies_ratings_list:
  if isinstance(rating, float):
    if rating > 7.5:
      movies_high_rated_flag = 1
      msg_text += str(movie) + '\nOriginal title: ' + str(
